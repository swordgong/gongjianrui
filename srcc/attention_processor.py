...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        use_suppression: bool = False,  # 添加一个标志来控制是否使用抑制注意力
        suppression_query_dim: Optional[int] = None,  # 抑制注意力的query维度
    ):
        super().__init__()
        ...
        # 添加抑制注意力机制，加强对擦除概念的抑制
        self.use_suppression = use_suppression
        if self.use_suppression:
            self.suppression_query_dim = suppression_query_dim if suppression_query_dim is not None else query_dim
            self.to_suppression_q = LoRACompatibleLinear(query_dim, self.inner_dim, bias=bias)
            self.to_suppression_k = LoRACompatibleLinear(self.cross_attention_dim, self.inner_dim, bias=bias)
            self.to_suppression_v = LoRACompatibleLinear(self.cross_attention_dim, self.inner_dim, bias=bias)
            self.suppression_scale = dim_head**-0.5 if self.scale_qk else 1.0
            self.suppression_alpha = nn.Parameter(torch.zeros(1))
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, attention_mask=None, **cross_attention_kwargs):
        ...
        # 如果使用抑制注意力，则计算抑制注意力并将其应用到hidden_states
        if self.use_suppression:
            suppression_query = self.to_suppression_q(hidden_states)
            suppression_key = self.to_suppression_k(encoder_hidden_states)
            suppression_value = self.to_suppression_v(encoder_hidden_states)

            suppression_query = self.head_to_batch_dim(suppression_query).contiguous()
            suppression_key = self.head_to_batch_dim(suppression_key).contiguous()
            suppression_value = self.head_to_batch_dim(suppression_value).contiguous()

            suppression_attention_scores = torch.bmm(suppression_query, suppression_key.transpose(-1, -2)) * self.suppression_scale
            suppression_attention_probs = suppression_attention_scores.softmax(dim=-1)
            suppression_hidden_states = torch.bmm(suppression_attention_probs, suppression_value)
            suppression_hidden_states = self.batch_to_head_dim(suppression_hidden_states)

            # 使用可学习的抑制强度参数
            hidden_states = hidden_states + self.suppression_alpha * suppression_hidden_states
