...
def load_chinese_conceptnet(file_path="chineseconceptnet.csv"):
    #加载中文 ConceptNet 数据并构建 NetworkX 图。
...
class BERTEmbedding:
    ...
    def __call__(self, entity_names):
        ...
        inputs = self.tokenizer(entity_names, padding=True, truncation=True, return_tensors="pt")
        with torch.no_grad():
            outputs = self.model(**inputs)
        # 使用 [CLS] token 的输出来表示实体嵌入
...
class MACEDataset(Dataset):
    ...
    def __init__(
        ...
        use_gpt=False,
        knowledge_graph=None,      # 知识图谱对象（NetworkX）
        ...
        if entity_embedding is None:
            self.entity_embedding = BERTEmbedding()  # 加载 BERT 实体嵌入模型
        else:
