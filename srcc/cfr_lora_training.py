...
from MACE.srcc.zmace_lora_atten_processor import LoRAAttnProcessor
from diffusers.loaders import AttnProcsLayers
from diffusers.optimization import get_scheduler
from diffusers.utils.import_utils import is_xformers_available
from tqdm.auto import tqdm
from transformers import AutoTokenizer, PretrainedConfig
from srcc.cfr_utils import *
from srcc.dataset import MACEDataset
import json
...
def collate_fn(examples, with_prior_preservation=False):
    ...
    entity_embeddings = [example["entity_embeddings"] for example in examples] # 新增
    ...
    entity_embeddings = torch.cat(entity_embeddings, dim=0) if entity_embeddings[0] is not None else None # 新增
    ...
    batch = {
        ...
        "entity_embeddings": entity_embeddings, # 新增
    }
    return batch
...
def main(args):
    ...
    # cf https://pytorch.org/docs/stable/notes/cuda.html#tensorfloat-32-tf32-o00pere-devices
    if args.allow_tf32:
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                with accelerator.accumulate(unet):
                    ...
                    if debug_once:
                        ...
                        debug_once = False
                    #新增
                    encoder_hidden_states = text_encoder(batch["input_ids"])[0] # 文本嵌入
                    entity_embeddings = batch["entity_embeddings"] # 实体嵌入    
                    if entity_embeddings is not None:
                        # 定义一个可学习的权重，用于控制实体嵌入的贡献
                        alpha = torch.sigmoid(torch.randn(1, device=accelerator.device))  # 使用 sigmoid 确保权重在 0 到 1 之间
                        encoder_hidden_states = alpha * encoder_hidden_states + (1 - alpha) * entity_embeddings#将知识图谱信息加权融合
